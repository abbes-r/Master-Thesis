...
4. **Scoring**: clusters are ranked by an expert-defined, feature-weighted score (co-designed with a marketing managers), then each airline's fuzzy memberships are combined into a single Digital Maturity Index and rescaled for a smooth, interpretable distribution.
